...
| Funil | `v_crm_funnel_daily` | Contagem por etapa a partir dos **eventos**, por **coorte de leads** (etapa `lead` filtrada pelo período; etapas seguintes contam sem trava de data, pela data de nascimento do lead) — ver `BANCO_DE_DADOS.md` seção 4.1 |
| Canais | `v_crm_events_enriched` | Leads por Entrada (`lead_entrada`) e por Origem (`lead_origem`) + cruzamento |
| Meta — Contas | `v_meta_account_daily` | Investido, Conversões Meta, Leads, CPL, Agendam., CPag por conta. Coorte por nascimento do lead desde 13/07 |
| Meta — Campanhas | `v_meta_campaign_daily` | Mesmo mix por campanha, com seletor de conta. Coorte por nascimento do lead desde 13/07 |
| Meta — Anúncios/Criativos | `v_meta_creative_daily` | Tabela (CTR, CPC, CPL, CPag) e grid visual de criativos, com **coluna de data** — permite o seletor de período (substituiu `v_meta_creative_performance`, que agregava tudo sem data) |
| Google — Campanhas/Gráfico | `v_google_campaign_daily` | Impressões/cliques/leads por dia, evolução de conversões, tabela de campanhas. Coorte por nascimento do lead desde 13/07 |
...
| **Operação** `/operacao` (agência, não client-facing) | `v_workflow_health_daily` + `v_client_workflow_health` | Saúde dos workflows n8n: tabela agregada (execuções/sucesso/erro/parcial por workflow+cliente, 14 dias) + feed das últimas 50 execuções. Só visível para usuários com acesso a mais de 1 cliente — trava imposta na própria view (`BANCO_DE_DADOS.md` seção 6.6), não só escondida no frontend |

### Seletor de período e metodologia de coorte

O seletor padrão do dashboard é **15 dias / 30 dias / 90 dias / Datas**
(mudou de 7/30/Datas em 13/07 — "7 dias" foi removido por deixar o CPA de
agendamento artificialmente ruim: poucos dias não dão tempo do lead
avançar no funil). Padrão ao abrir: 30 dias.

Como o Funil e as views de mídia (Meta/Google) usam **coorte de leads**
(uma pessoa é contada no período em que *nasceu* como lead; avanços
posteriores dela contam nessa mesma safra, não na data em que aconteceram),
períodos curtos sempre vão mostrar menos conversão que períodos longos —
isso é esperado, não é bug. O componente `components/CohortNote.tsx`
(banner retrátil, presente nas abas Funil, Meta Ads e Google Ads) explica
isso para quem está lendo o dashboard.
...
### Rota `/operacao` — a única que não é por cliente

`/operacao` foge do padrão `/clientes/[slug]/dashboard`: é uma página
**agência-wide** (soma dados de todos os clientes do usuário), então não
faz sentido ter slug de cliente na URL. Acesso: o link só aparece no
frontend para quem tem mais de 1 cliente (`multiClient`, mesmo estado usado
para mostrar "Trocar cliente"), mas a segurança de verdade está no banco —
a view `v_workflow_health_daily` só retorna linha para esse mesmo grupo,
mesmo se alguém tentar acessar a URL direto ou consultar a view via
`supabase-js` fora do app. Ver `BANCO_DE_DADOS.md` seção 6.6.
