...
**Última atualização:** 17/07/2026 — revisão conceitual completa V2 (15/07),
remoção do backfill Clinicorp da Royal (16/07), otimização de performance das
RPCs (16/07), e publicação do frontend V2 (17/07). Ver seções 12-16 abaixo.
Base anterior (13/07): backfill retroativo de receita (seção 4.5).
a partir de export do Clinicorp — 25 eventos `ganho` + 8 `agendado`,
R$ 125.551,03, para pacientes já existentes no CRM que nunca tiveram o
avanço de funil registrado por falha de processo comercial (não de
sistema). Achado estratégico: 86% da base de pacientes pagantes da Royal
(160 de 187) nunca passou pelo funil digital — fora de escopo do dashboard
por definição. Ver seção 4.5. Base anterior, mesma data: auditoria
completa de coorte em todo o dashboard + lógica de coorte estendida para
views de mídia. Ver seção 4.4. Base anterior, 11/07/2026: cliente
`[TEMPLATE]` populado com cenário fictício completo para demonstração —
ver seção 10; e nova tabela `workflow_execution_logs` + 2 views de saúde
de workflow, observabilidade do pipeline n8n — ver seção 2.5. Base
anterior, 09/07/2026: mudança de metodologia do funil comercial +
investigação de duplicidade de eventos + gap de atribuição Meta Ads em
leads de Form nativo, com backfill via Meta Lead Center concluído).
...
| `v_workflow_health_daily` **(novo, 11/07)** | cliente + dia + workflow | Agregação (`count`/`avg duration_ms`) por `workflow_execution_logs`, uso interno da agência — **sem RLS por client_id de propósito**, não é pra ir pro dashboard do cliente final |
...
## 4.5 Backfill retroativo de receita — export do Clinicorp (13/07/2026)

**Contexto:** o time comercial da Royal ficou meses (nov/2025 a jul/2026)
sem disparar corretamente as tags de WhatsApp no GHL que geram os eventos
de avanço de funil — então pacientes que **efetivamente pagaram e fecharam
procedimento** nunca tiveram `agendado`/`ganho` registrados no CRM.
Diferente das seções 4.1-4.4 (bugs de metodologia/normalização), esta é
falha de **processo humano**, não de sistema — mas o resultado prático
(receita ausente no dashboard) é o mesmo, e a correção é retroativa via
dado externo, no mesmo espírito do backfill do Meta Lead Center (seção
4.3).

**Fonte:** export manual do Clinicorp (sistema de gestão da clínica),
planilha de faturamento com paciente, telefone, data, procedimento e
valor.

**Achado no processamento do arquivo, antes de qualquer cruzamento:** o
export trazia **1084 linhas**, mas a última era uma **linha de total do
próprio relatório** (`Valor R$ = R$ 1.018.849,08`, sem nome/telefone/data)
— confirmado matematicamente: a soma das outras 1083 linhas batia com
esse valor a 14 centavos de diferença (arredondamento). Se não tivesse
sido excluída, o valor total teria sido contado em dobro (R$ 2.037.698,30
em vez de R$ 1.018.849,22). **Lição:** sempre somar e conferir contra
algum total de controle antes de processar um export financeiro de
terceiros — nesse caso a própria matemática denunciou o erro.

**Números reais do arquivo (corrigido):** 1083 linhas de procedimento →
**187 pacientes únicos** por telefone → **R$ 1.018.849,22** no total,
período 07/11/2025 a 10/07/2026.

**Cruzamento por telefone contra `events_normalized` (Royal):**
- **27 pacientes** já existiam como contato no CRM (14% do total) →
  R$ 154.500,28
- **160 pacientes NÃO existem em lugar nenhum do CRM** (86%) →
  R$ 864.348,94 — **achado estratégico, não só técnico**: a grande
  maioria da receita real da clínica não passa pelo funil digital
  (GHL/marketing) — é indicação, paciente antigo, walk-in, etc. Decisão
  do usuário: **fora de escopo por definição de propósito do produto** —
  o dashboard mede aquisição digital, não faturamento total da clínica;
  esses 160 permanecem só no Clinicorp, não entram no banco.

**Dos 27 que bateram, ajuste fino:**
- **2 já tinham `ganho`** registrado (Dionatham, Cássia) → tratados como
  coincidentes, não duplicados.
- **17 já tinham `agendado`** → só o `ganho` foi criado para esses.
- **8 não tinham `agendado`** → criado também, na data mais antiga do
  conjunto de procedimentos daquele telefone.

**Caso de atenção identificado e não bloqueante:** um telefone
(`556198831536`) pertence a duas pessoas diferentes no export do
Clinicorp (Danielle e Gabriel, provavelmente mãe e filho — mesmo dia de
atendimento). Como o CRM só tem 1 contato por telefone, o valor de ambos
foi somado no mesmo evento — o dinheiro é real e o telefone é o canal de
contato correto; só o nome não representa 100% de quem fez qual
procedimento. Aceito como limitação de dado, não como erro.

**Metodologia de agregação (para não repetir o erro da seção 4.2):**
como o mesmo paciente podia ter vários procedimentos na mesma visita
(inclusive 2 datas de visita distintas para 17 dos 187), a agregação foi
feita **por telefone**, não por linha — 1 evento `ganho` por paciente,
`valor_ganho` = soma de todos os procedimentos, `procedimento_ganho` =
lista de procedimentos únicos, data do evento = a mais recente do
conjunto (para `ganho`) / mais antiga (para `agendado`). Isso evita a
armadilha que a própria blindagem de `DISTINCT ON` da seção 4.2 causaria
se cada procedimento virasse um evento `ganho` separado (contaria só o
mais recente, perdendo receita real).

**Rastreabilidade:** todos os 33 eventos inseridos (25 `ganho` + 8
`agendado`) marcados com `source_event_type = 'manual_backfill_clinicorp'`
e `source_system = 'manual'` — nome específico o suficiente para não se
confundir com o backfill de 08/07 (`clean_rebuild_from_ghl_contacts_export`)
numa investigação futura.

**Resultado validado em produção:** `count = 33`, `sum(valor_ganho) =
125551.03` — bateu exatamente com o esperado.
...
## 12. Revisão conceitual V2 — 15/07/2026

A revisão de 15/07 não foi só uma correção de números. Ela redefiniu a
metodologia analítica do ImpulsHub para que leads, eventos, oportunidades,
vendas, receita, mídia e atribuição sejam tratados de forma consistente em
qualquer cliente. Os princípios consolidados:
...
| Performance | Cliente por data | Consolida mídia, coorte e atividade comercial |

**Mudanças conceituais principais:**

- `event_code` determina o fato oficial. `pipeline_stage` e `status` são
  auditoria — não substituem o evento normalizado.
- Pessoa ≠ Oportunidade: uma pessoa pode ter várias compras/oportunidades.
  Receita não é mais deduplicada por contato.
- `NULL` ≠ zero: receita ausente permanece `NULL`. ROAS/CAC só aparecem
  quando `revenue_is_complete = true`.
- Atribuição estritamente técnica: `meta_ad_id`, `google_campaign_id`,
  `gclid` etc. Nunca por texto, nome de campanha ou origem livre.
- Coorte vs. diário separados formalmente: funil/leads/canais/mídia usam
  coorte (data do lead); diário/eventos/vendas fechadas usam data real.
- Uma view canônica por canal de mídia (não uma por nível de agrupamento).

**Objetos V2 criados e validados:**

```
CRM canônico
  v_crm_events_feed_v2      — feed seguro, horário local, campos técnicos
  v_crm_events_daily_v2     — atividade diária por fato
  v_crm_lead_journey_v2     — jornada canônica com datas dos marcos
  v_client_leads_by_stage_v2 — estágio atual, flags e atribuição
  v_crm_funnel_daily_v2     — funil cumulativo por coorte
  v_crm_channels_daily_v2   — plataforma técnica, entrada e origem separadas
...
Mídia canônica
  v_meta_ads_v2             — anúncio por dia, toda a hierarquia na linha
  v_google_ads_v2           — anúncio por dia
  v_google_keywords_v2      — palavra-chave por dia

Consolidado oficial
  v_client_performance_daily_v2
  get_client_overview_v2()

Qualidade (separadas, não alimentam métricas principais)
...
**Todas as 17 views V2 têm `security_invoker = true`.** A função
`get_client_overview_v2` não é `SECURITY DEFINER`. As tabelas base têm
RLS ativa com política por `client_id`.

---

## 13. Funções RPC de mídia detalhada — 15-16/07/2026

Para exibir métricas de mídia + CRM cruzadas por campanha/anúncio/criativo
no dashboard, sem risco de truncamento (a Royal tem ~4.294 linhas de Meta
por período de 90 dias), foram criadas duas funções oficiais:

### `get_meta_ads_summary_v2(p_client_id, p_start_date, p_end_date, p_dimension)`

**`p_dimension`:** `'account'` | `'campaign'` | `'adset'` | `'ad'` | `'creative'`

Retorna por linha:
- Chaves e nomes da dimensão
- Mídia: `spend`, `impressions`, `clicks`
- CRM (por coorte de lead): `crm_leads`, `crm_agendados`, `crm_ganhos`
- Financeiro: `acquisition_revenue`, `acquisition_revenue_is_complete`,
  `roas_acquisition`

**Decisão de arquitetura:** o nível `creative` usa um ramo separado
(`UNION ALL`) — agrega direto por `creative_id` real de cada linha (mídia
pura, 100% correta), enquanto os demais níveis agregam primeiro por `ad_id`
(grão onde a atribuição de CRM é inequívoca) e depois somam para cima.
Isso evita duplicação de leads quando um anúncio tem mais de um criativo.

**Auditoria confirmada:** nenhum `ad_id` com mais de um `creative_id`
encontrado na base atual. O `ad_name` retorna em 100% das 79 linhas de
criativos da Royal.

### `get_google_ads_summary_v2(p_client_id, p_start_date, p_end_date, p_dimension)`

**`p_dimension`:** `'account'` | `'campaign'` | `'ad_group'` | `'ad'`

CRM confiável apenas em `account` e `campaign` (único nível onde
`google_campaign_id` está sempre presente). Em `ad_group`/`ad`, CRM = NULL.
Grupos e anúncios sem ID técnico aparecem como buckets explícitos ("Sem
grupo identificado") — nunca somem dos totais.

**Segurança:** ambas as funções são `SECURITY INVOKER STABLE` — respeitam
RLS das tabelas base. Um `client_id` ao qual o usuário não tem acesso
simplesmente retorna vazio.

---

## 14. Remoção do backfill Clinicorp da Royal — 16/07/2026

**Decisão:** não liberar o histórico Clinicorp junto com a metodologia atual
do GHL. O Clinicorp tem 25 ganhos e ~R$ 125.551 de receita sem
`opportunity_id` — promovê-los a fato oficial misturaria grãos e poderia
distorcer métricas de aquisição.

**O que foi removido:**
- 25 eventos de ganho (source: `manual_backfill_clinicorp`)
- 8 eventos de agendamento
- 33 eventos brutos vinculados

**Como foi feito:** validação antes do DELETE + transação única + backup
integral em `private.backfill_archive`. Nenhuma alteração em views, RPCs
ou metodologia. Nenhum impacto nos demais eventos da Royal.

**Estado atual da Royal (pós-remoção, 16/07):**

| Etapa | Contagem |
|---|---|
| Leads | 2.217 |
| Primeiras conversas | 1.270 |
| Agendados | 550 |
| Ganhos | 8 |
| Perdidos | 369 |

**Plano futuro:** quando o time começar a operar corretamente no GHL, marcar
uma data de corte e importar o Clinicorp até essa data uma única vez. Dali
em diante, apenas a operação normal alimenta vendas e receita.

---

## 15. Otimização de performance das RPCs — 16/07/2026

Após a criação das funções de mídia detalhada, a `get_meta_ads_summary_v2`
apresentava lentidão (2.350ms, uso de disco temporário). Foram aplicadas
três melhorias:

**Índices criados em `events_normalized`:**
- `(client_id, contact_id, event_datetime, received_at)`
- Índice filtrado somente para `event_code = 'lead'`

**Índice criado em `meta_ads_daily`:**
- `(client_id, ad_id, date DESC, updated_at DESC)`

**Otimização de `v_crm_lead_journey_v2`:**
- O bloco de atribuição passou a consultar apenas `event_code = 'lead'`
  em vez de percorrer todos os eventos.

**`work_mem = 16MB`** aplicado dentro da RPC Meta para evitar uso de disco.

**Resultado:**

| Métrica | Antes | Depois |
|---|---|---|
| Tempo | 2.350 ms | 137 ms |
| Temp read/written | 797/798 | 0/0 |
| Shared buffers | 40.234 | 33.407 |

**94,2% de redução** — aproximadamente 17× mais rápido, sem disco temporário.

---

## 16. Arquitetura final e regras de manutenção
...
VIEWS CANÔNICAS
  CRM por evento, lead, oportunidade e venda (9 views)
  Meta Ads por anúncio/dia  (v_meta_ads_v2)
  Google Ads por anúncio/dia (v_google_ads_v2)
  Keywords por palavra-chave/dia (v_google_keywords_v2)
...
**Regra de manutenção — antes de criar qualquer view nova:**
> "Existe mudança real de grão, regra canônica indispensável, necessidade
> de segurança ou reutilização clara? Se a resposta for não, a view não
> deve ser criada."

**Legado congelado:** as views antigas (`v_crm_events_enriched`,
`v_crm_funnel_daily`, `v_meta_account_daily`, etc.) foram marcadas como
depreciadas. Não devem receber novas regras. Serão removidas após
confirmação de que o frontend não as referencia mais.

**Checklist de segurança (estado 17/07):**
- [x] RLS ativa nas tabelas base
- [x] 17 views V2 com `security_invoker = true`
- [x] Funções RPC com `SECURITY INVOKER`
- [x] `clients_base` sem exposição de tokens
- [x] `v_workflow_health_daily` protegida contra vazamento cross-cliente
- [ ] Remoção formal das views antigas (pós-migração)
- [ ] Consolidar views de qualidade em fonte única (backlog)
