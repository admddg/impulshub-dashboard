...
const migrationPath = new URL('../supabase/migrations/20260926000000_imp213_role_visibility.sql', import.meta.url)
...
test('events_normalized expõe somente colunas operacionais a authenticated', () => {
  assert.match(sql, /revoke select on table public\.events_normalized from public, anon, authenticated/)
...
test('histórico continua legível para membro e mascara somente campos financeiros', () => {
  assert.match(sql, /where "gated"\."client_id" in \(select "client_id" from private\.financial_client_ids\(\)\)/)
  ...
  assert.match(sql, /case when "m"\."kind" = 'revenue'::"text"[\s\S]*then null::"text" else "m"\."evidence" end/)
})
