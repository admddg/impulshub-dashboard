-- IMP-226 fase 1: arquivo de aplicacao manual.
-- Executar somente apos revisao e autorizacao de producao.

-- IMP-226 fase 1: toda conversa individual nova abre oportunidade.
-- Grupos, LID e mensagens incompletas continuam sendo filtrados antes desta funcao.
-- A unica mudanca em relacao a crm.stevo_parse_messages atual e a condicao de criacao da oportunidade.

begin;
set local lock_timeout = '5s';

CREATE OR REPLACE FUNCTION crm.stevo_parse_messages(p_limit integer DEFAULT 20000)
 RETURNS TABLE(lidos integer, contatos_criados integer, oportunidades_criadas integer, atendimentos integer, ignorados_grupo integer, ignorados_lid integer, ignorados_duplicados integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
values ('20260928000002', 'imp226a_open_card_on_individual_conversation')
