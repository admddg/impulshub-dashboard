-- IMP-217: outcome value/currency/reason reach dashboard and eligible outbox.
-- Decision 8: dashboard always receives ganho; pending ganho creates no Purchase.
-- Decision 9: no post-gain value-fill emission path (out of scope today).
-- Decision 10: events_normalized.currency follows the table's existing English
-- naming convention; current CRM currency is BRL.
...
revoke all on function public.crm_register_won(uuid, text, integer, numeric, text) from public, anon, authenticated, service_role;
...
revoke all on function public.crm_register_lost(uuid, text, integer, text) from public, anon, authenticated, service_role;
grant execute on function public.crm_register_lost(uuid, text, integer, text) to authenticated, service_role;
create index if not exists events_normalized_crm_opportunity_idx on public.events_normalized (client_id, opportunity_id, event_code) where source_system = 'impuls_crm';
insert into supabase_migrations.schema_migrations (version, name) values ('20261003000000', 'imp217_outcome_value_reason') on conflict (version) do nothing;
do $gate$
begin
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='events_normalized' and column_name='currency') then raise exception 'IMP217_GATE: currency ausente'; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='events_normalized' and column_name='valor_ganho') then raise exception 'IMP217_GATE: valor_ganho ausente'; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='events_normalized' and column_name='motivo_perda_categoria') then raise exception 'IMP217_GATE: motivo ausente'; end if;
  if not exists (select 1 from pg_indexes where schemaname='public' and indexname='events_normalized_crm_opportunity_idx') then raise exception 'IMP217_GATE: dedupe ausente'; end if;
  if has_function_privilege('anon', 'crm.emit_opportunity_stage_event()', 'EXECUTE') then raise exception 'IMP217_GATE: anon executa emissor'; end if;
end
$gate$;
