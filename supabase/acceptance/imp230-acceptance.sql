...
begin
  ...
  v_result := crm.intake_form_lead(v_slug,v_token,'IMP230 aceite','5511999999999','imp230-accept@example.invalid','gclid-imp230','gbraid-imp230','wbraid-imp230','google','cpc','campanha','conteudo','termo','https://example.invalid/landing');
  v_result := crm.intake_form_lead(v_slug,v_token,'IMP230 aceite','5511999999999','imp230-accept@example.invalid','gclid-imp230','gbraid-imp230','wbraid-imp230','google','cpc','campanha','conteudo','termo','https://example.invalid/landing');
  ...
  if has_table_privilege('anon','crm.contacts','INSERT') or has_table_privilege('anon','crm.opportunities','INSERT') or has_function_privilege('anon','crm.intake_form_lead(text,uuid,text,text,text,text,text,text,text,text,text,text,text,text)','EXECUTE') then raise exception 'IMP230_ACCEPT: ACL anon aberta'; end if;
  begin perform crm.intake_form_lead(v_slug,gen_random_uuid(),'nao deve criar','5511888888888','imp230-invalid@example.invalid'); raise exception 'IMP230_ACCEPT: token invalido aceito'; exception when others then if sqlerrm like 'IMP230_ACCEPT:%' then raise; end if; end;
