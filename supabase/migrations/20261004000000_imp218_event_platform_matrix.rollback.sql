-- IMP-218 rollback. Restores the pre-IMP-218 trigger definition and matrix.
-- Outbox rows written during the interval are intentionally preserved.
-- The three outbox columns remain nullable on rollback: reverting them to NOT NULL
-- would require deleting the preserved Google rows, which is expressly forbidden.
...
drop index if exists public.conversion_outbox_event_platform_uidx;
alter table crm.event_map drop column if exists meta_event_name, drop column if exists meta_event_name_whatsapp, drop column if exists google_event_name;
delete from supabase_migrations.schema_migrations where version = '20261004000000';

do $rollback_gate$
begin
  if exists (select 1 from information_schema.columns where table_schema='crm' and table_name='event_map' and column_name='google_event_name') then raise exception 'IMP218_ROLLBACK: matriz nao removida'; end if;
  if exists (select 1 from pg_indexes where schemaname='public' and indexname='conversion_outbox_event_platform_uidx') then raise exception 'IMP218_ROLLBACK: indice nao removido'; end if;
  if has_function_privilege('anon', 'crm.emit_opportunity_stage_event()', 'EXECUTE') then raise exception 'IMP218_ROLLBACK: anon executa emissor'; end if;
end
$rollback_gate$;
